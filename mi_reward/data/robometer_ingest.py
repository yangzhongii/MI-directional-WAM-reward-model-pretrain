...
def ingest_robometer(
    ...
    unmatched = 0
    observed_tasks: Counter[str] = Counter()
    observed_quality_labels: Counter[str] = Counter()
    for dataset_name_value in dataset_names:
        ...
        for index in tqdm(
            range(limit),
            desc=f"Robometer ingest: {dataset_name}",
            unit="trajectory",
        ):
            row = dataset[index]
            ...
            quality = str(row.get("quality_label") or "").strip().lower()
            observed_tasks[task or "<missing>"] += 1
            observed_quality_labels[quality or "<missing>"] += 1
    ...
    if not records:
        task_summary = ", ".join(
            f"{task!r} ({count})" for task, count in observed_tasks.most_common(8)
        )
        quality_summary = ", ".join(
            f"{label!r} ({count})" for label, count in observed_quality_labels.most_common()
        )
        raise ValueError(
            "Robometer ingestion produced no usable base trajectories. "
            f"matched_rows={len(pending)}, unmatched_rows={unmatched}, "
            f"configured_success_labels={sorted(success_labels)}. "
            f"Observed quality labels: {quality_summary or '<none>'}. "
            f"Observed tasks: {task_summary or '<none>'}."
        )
    ...
    report = {
        ...
        "matched": len(pending),
        "observed_quality_labels": dict(observed_quality_labels),
        "observed_tasks": dict(observed_tasks),
...
def main() -> None:
    ...
    config = payload.get("robometer")
    if config is None and isinstance(base_data, dict):
        config = base_data.get("robometer")
    if config is None:
        config = payload
