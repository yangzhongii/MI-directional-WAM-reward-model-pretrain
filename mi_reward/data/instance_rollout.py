...
def ingest_instance_rollout_candidates(
    ...
    *,
    split: str = "train",
    require_scene_variant: bool = False,
    ...
    for record in tqdm(records, desc="Candidate verification", unit="candidate"):
        ...
        if require_scene_variant and scene_variant is None:
            record_reasons.append("missing_scene_variant")
        try:
            simulation = (
        ...
        instance_checks = dict(instance_result.checks)
        instance_reasons = [*record_reasons, *instance_result.reasons]
        generator = str(record.get("generator", "instance_rollout"))
        if generator.startswith("cosmos_predict"):
            visual_consistency = record.get("visual_consistency")
            visual_ok = bool(
                isinstance(visual_consistency, dict)
                and visual_consistency.get("passed") is True
            )
            instance_checks["visual_rollout_consistency"] = visual_ok
            if not visual_ok:
                instance_reasons.append("visual_rollout_inconsistent_with_simulation")
        instance_verification = VerificationRecord(
            status="accepted" if instance_checks and all(instance_checks.values()) else "rejected",
            checks=instance_checks,
            reasons=sorted(set(instance_reasons)),
        ...
        raw_outcome = record.get("task_outcome")
        if isinstance(raw_outcome, dict):
            task_outcome = TaskOutcome.from_dict(raw_outcome)
        else:
            outcome_checks = instance_result.outcome_checks or {}
            profile = str((record.get("metadata") or {}).get("candidate_profile", "unknown"))
            failure_mode = instance_result.failure_mode
            if failure_mode == "goal_not_reached" and profile in {
                "undershoot", "overshoot", "wrong_direction", "regress_after_progress"
            }:
                failure_mode = profile
            task_outcome = TaskOutcome(
                success=bool(outcome_checks) and all(outcome_checks.values()),
