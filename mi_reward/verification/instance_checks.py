...
def verify_instance_candidate(
    *,
    ...
    checks["collision_free"] = bool((collision >= 0.5).all().item())
    checks["contact_valid"] = bool((contact >= 0.5).all().item())
    checks["target_satisfied"] = bool(terminal >= 0.5)
    if not checks["collision_free"]:
        reasons.append("collision_detected")
    if not checks["contact_valid"]:
        reasons.append("invalid_contact_phase")
    if not checks["target_satisfied"]:
        reasons.append("task_goal_not_satisfied")
    return InstanceCheckResult(checks=checks, reasons=sorted(set(reasons)))
