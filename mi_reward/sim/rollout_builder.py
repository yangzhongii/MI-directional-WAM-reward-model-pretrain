...
def write_rollout_artifacts(
    ...
    *,
    ...
    if not (steps == len(robot_states) == len(object_states) == len(relations.values)):
        raise ValueError("Rollout frames, states, objects, and relations must share a timestep count.")
    if len(rollout.actions) not in {steps, max(steps - 1, 0)}:
        raise ValueError("Rollout actions must be frame-aligned [T,A] or transition-aligned [T-1,A].")
