...
from world_model.inference.cosmos_action import build_action_inference, step_inference_dual_view
...
def _samples(dataset_root: Path, *, max_episodes: int, fps_downsample_ratio: int, chunk_size: int,
    ...
    for annotation_path in annotations:
        ...
        videos = {
            str(item.get("camera")): dataset_root / str(item["video_path"])
            for item in annotation.get("videos", [])
        }
        if "agentview" not in videos or "wrist" not in videos:
            raise ValueError(f"Dual-view annotation is incomplete: {annotation_path}")
        references = {
            "third_view": _video_frames(videos["agentview"], frame_ids, resolution),
            "wrist_view": _video_frames(videos["wrist"], frame_ids, resolution),
        }
        samples.append({
...
def run(config_path: Path, root: Path, *, max_episodes_override: int | None = None) -> dict[str, Any]:
    ...
    per_mode: dict[str, dict[str, list[dict[str, Any]]]] = {}
    for mode, use_lora, checkpoint in (
        ...
        mode_metrics: dict[str, list[dict[str, Any]]] = {"third_view": [], "wrist_view": []}
        try:
            for index, sample in enumerate(tqdm(samples, desc=f"Cosmos held-out {mode}", unit="episode")):
                _, streams = step_inference_dual_view(
                    inference,
                    sample["references"]["third_view"][0],
                    sample["references"]["wrist_view"][0],
                    action=sample["actions"],
                    guidance=int(evaluation.get("guidance", 7)),
                    seed=int(evaluation.get("seed", 0)) + index,
                )
                predictions[sample["episode_id"]][mode] = streams
                for view_name, generated in streams.items():
                    reference = sample["references"][view_name]
                    generated = np.asarray(generated[..., :3], dtype=np.uint8)
                    if generated.shape != reference.shape:
                        raise ValueError(
                            f"{mode}/{view_name} returned {generated.shape}, expected {reference.shape}"
                        )
                    mode_metrics[view_name].append({
                        "episode_id": sample["episode_id"], **_metrics(generated, reference)
                    })
                    mode_dir = output / mode / view_name
                    mode_dir.mkdir(parents=True, exist_ok=True)
                    imageio.mimwrite(
                        mode_dir / f"{sample['episode_id']}.mp4", generated,
                        fps=4, codec="libx264", quality=8,
                    )
        finally:
            inference.cleanup()
            del inference
    ...
    for sample in samples:
        ...
        for view_name, reference in sample["references"].items():
            comparison = np.concatenate(
                (
                    reference,
                    predictions[episode_id]["base"][view_name],
                    predictions[episode_id]["adapted"][view_name],
                ),
                axis=2,
            )
            comparison_dir = output / "comparisons" / view_name
            comparison_dir.mkdir(parents=True, exist_ok=True)
            imageio.mimwrite(
                comparison_dir / f"{episode_id}.mp4", comparison,
                fps=4, codec="libx264", quality=8,
            )
    aggregate = {
        mode: {view_name: _aggregate(values) for view_name, values in views.items()}
        for mode, views in per_mode.items()
    }
    gates = {
        view_name: _quality_gate(
            aggregate["base"][view_name], aggregate["adapted"][view_name],
            dict(evaluation.get("quality_gate", {})),
        )
        for view_name in ("third_view", "wrist_view")
    }
    report = {
        "status": "complete",
        ...
        "camera_layout": ["third_view", "wrist_view"],
        "output": str(output),
    }
