...
def test_instance_rollout_rejects_collision_contact_and_goal_failures(tmp_path: Path) -> None:
    for name, settings, reason in (
        ("collision", {"collision": True}, "collision_detected"),
        ("contact", {"contact": False}, "invalid_contact_phase"),
        ("goal", {"terminal": False}, "task_goal_not_satisfied"),
    ):
        ...
        assert reason in example.verification.reasons
